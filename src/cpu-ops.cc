...
//
//
template <regw R, mode M>
inline void load()
{
    (this->*R)(read<M>());
}

template <regr R, mode M>
inline void store()
{
    write((this->*R)(), (this->*M)());
}

template <regr from, regw to>
...
//
template <CPU::Flag F>
inline uint8_t if_clear()
{
    return !(_P & F);
}

template <CPU::Flag F>
inline uint8_t if_set()
{
    return _P & F;
}
...
{
    uint8_t m = read<M>();
    int r = _A + m + !!(_P & C_FLAG);
    set_if<V_FLAG>((~(_A ^ m) & (_A ^ r)) & 0x80);
    set_if<C_FLAG>(r >> 8);
    setZN(_A = r);
}
...
//
//
template <CPU::Flag from, CPU::Flag to>
inline void copyflag()
{
    set_if<to>(_P & from);
}

template <op F1, op F2, int I = 1>
inline void unofficial()
{
    (this->*F1)();
    _PC -= I; // bleh
    (this->*F2)();
